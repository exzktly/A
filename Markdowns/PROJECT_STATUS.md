...
| **15** | Centre stack v2 native — outer `app._notebook` + nested `app._plotting_notebook` replaced with `NamedPageStack` (`QStackedWidget` subclass); `_GroupedTabBar` deleted; every centre-stack caller routed through `currentName()` / `setCurrentByName()` / `pageNames()`. Plan: `design/plans/phase_15_named_page_stack.md`. | ✅ shipped (pending runtime QA) |

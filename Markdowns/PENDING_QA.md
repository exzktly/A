...
| `4365c9f` | Phase 11b slice — global channel chip is the source of truth | In Plotting, edit the channel via the ctxbar combo. The active renderer redraws against the new channel; switch plot type and the chip mirrors the new renderer's combo. |
| `4b083ac` … `60aabc0` | **Phase 15** — centre stack now `NamedPageStack` | RailNav click sweep: all 8 sections render (Plotting / smFISH / Statistics / Image Table / Segmentation / Review CSV / Sample Definitions / Batch Export). Plotting ctxbar SegmentedControl click sweep: all 5 plot types render; channel chip mirrors active renderer's combo. Batch Export entry points (line / bar / scatter cells / scatter agg) still land on Batch Export and set the correct mode. smFISH single-well clamp still fires when clicking a row letter. Movie Montage preview refresh still triggers. No regressions in Sample Definitions / Image Table / Stats sidebar visibility under `_on_tab_change`. |
| `f8adb1c` | Phase 15.1 — flatten `smfish_tab.py` into `tabs/smfish_tab_view.py` | Open the smFISH tab on a dataset with smFISH channels: combo boxes populate (channel / FOV / timepoint), the image renders, overlay toggle works, threshold edit re-renders, CDF popup opens and tracks the threshold line, "Apply Global Threshold" kicks off the background worker and finishes with the info dialog. Switch to a dataset without smFISH channels: tab shows the "No smFISH data" message and disables controls. |
| `618bc23` | Phase 15.1 — flatten `cell_gating_tab.py` into `tabs/cell_gating_tab_view.py` | Sample Definitions → Cell Gating sub-tab opens, shows the cell-area CDF + per-channel FluorGating rows for each loaded channel. Editing the cell-area threshold or any FluorGating value kicks off the background `GatingWorker` (progress bar in the statusbar advances 1/N…N/N); on finish, the plot redraws. ThreshFracOn edits persist to `pipeline_info.json` and re-load on dataset re-open. Clear / Reset on the Sample Definitions panel resets all three (area, FluorGating, ThreshFracOn) back to defaults. |

    ...
    *,
    ...
    if active_rsets:
        for idx, rset in enumerate(active_rsets):
            ...
            if agg_times:
                app._ax_mean.plot(agg_times, agg_means, color=color, lw=2, marker="o", markersize=4, label=lbl_str, zorder=3)
                app._ax_mean.fill_between(agg_times, [m - e for m, e in zip(agg_means, agg_errs)], [m + e for m, e in zip(agg_means, agg_errs)], color=color, alpha=0.15, zorder=2)
                vf = [(t, f) for t, f in zip(agg_times, agg_fracs) if not math.isnan(f)]
                if vf:
                    vt2, vf2 = zip(*vf)
                    app._ax_frac.plot(vt2, vf2, color=color, lw=2, marker="s", markersize=3, label=lbl_str, zorder=3)
                    app._ax_frac.fill_between(vt2, 0, vf2, color=color, alpha=0.10, zorder=2)
                any_ts = True
            if all_fluor_vals_rset:
                fluor_s = sorted(all_fluor_vals_rset)
                n = len(fluor_s)
                app._ax_cdf.plot(fluor_s, [(k + 1) / n for k in range(n)], color=color, lw=1.8, label=f"{lbl_str} (n={n:,})", zorder=3)
                any_cdf = True
    else:
        cell_area_threshold = app._get_cell_area_threshold()
        fluor_gates = app._get_all_fluor_gates()
        for i, label in enumerate(selected):
            ...
            if pts:
                ...
                if vm:
                    vt, vmm, vs = zip(*vm)
                    app._ax_mean.plot(vt, vmm, color=color, lw=2, marker="o", markersize=4, label=disp, zorder=3)
                    app._ax_mean.fill_between(vt, [m - s for m, s in zip(vmm, vs)], [m + s for m, s in zip(vmm, vs)], color=color, alpha=0.15, zorder=2)
                vf = [(t, f) for t, f in zip(times, fracs) if not math.isnan(f)]
                if vf:
                    vt2, vf2 = zip(*vf)
                    app._ax_frac.plot(vt2, vf2, color=color, lw=2, marker="s", markersize=3, label=disp, zorder=3)
                    app._ax_frac.fill_between(vt2, 0, vf2, color=color, alpha=0.10, zorder=2)
                any_ts = True
            ...
            if vals:
                n = len(vals)
                app._ax_cdf.plot(vals, [(k + 1) / n for k in range(n)], color=color, lw=1.8, label=f"{disp} (n={n:,})", zorder=3)
                any_cdf = True

    if any_ts:
        app._ax_mean.axhline(threshold, color=warn, lw=1.0, ls="--", alpha=0.8, zorder=1)
        leg_mean = app._ax_mean.legend(**legend_kw)
        leg_frac = app._ax_frac.legend(**legend_kw)
    ...
    if any_cdf:
        app._ax_cdf.axvline(threshold, color=warn, lw=1.2, ls="--", label=f"threshold={threshold:.2f}", zorder=4, picker=8)
        try:
        ...
        if cdf_hi <= cdf_lo:
            cdf_hi = cdf_lo + 1.0
        app._ax_cdf.axvspan(threshold, cdf_hi, alpha=0.05, color=warn, zorder=1)
        leg_cdf = app._ax_cdf.legend(**legend_kw)
        leg_cdf.set_draggable(True)
        leg_cdf.set_visible(app._legend_visible["cdf"])
        app._ax_cdf.set_xlim(cdf_lo, cdf_hi)
    else:
        app._ax_cdf.text(0.5, 0.5, f"No {app._active_channel.upper()} data found.", transform=app._ax_cdf.transAxes, ha="center", va="center", color=txt_mut, fontsize=10)

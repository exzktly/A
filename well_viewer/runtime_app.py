...
class WellViewerApp(QWidget):
    ...
    def _toggle_swarm(self) -> None:
        self._bar_swarm.set(not bool(self._bar_swarm.get()))
        self._redraw_bars()

    def _toggle_violin(self) -> None:
        self._bar_violin.set(not bool(self._bar_violin.get()))
        self._redraw_bars()

    def _toggle_log_scale(self) -> None:
        self._bar_log_scale.set(not bool(self._bar_log_scale.get()))
        self._redraw_bars()
    ...
    def _update_tp_selection_display(self) -> None:
        if hasattr(self, "_scatter_agg_tp_label"):
            selected = [k for k, holder in self._scatter_agg_tp_selections.items() if holder.get()]

...
collapsible** (default expanded, toggle in the titlebar — see §8 Q1). Every
other contradiction reconciles toward the mockup body markup.
...
| C2 | `DECISIONS_NEEDED.md` #4 | "delete theme combo; single dark theme" → re-resolve to **Dark / Light** segmented in titlebar (System optional — Q2). |
...
| C8 | `PORT_PLAN.md` Table 1 row 41 ("Theme combo … delete") | Reverse — keep as titlebar segmented. |
...
| C16 | `DESIGN_NOTES.md` §6.5 | Re-instate `Dark / Light` (± System) titlebar segmented. |
...
| **`TitleBar` (v2 chrome)** | extend existing | Native frame (per current DECISIONS #4); adds breadcrumb area, mono file chip, version pill, theme `SegmentedControl`, refresh `IconButton`, kbd-hinted `Open` primary button, rail-collapse toggle (for A6/B23). | extend current header builder in `all_well.py`; expose `set_breadcrumb(parts: list[str])`, `set_file(path: str)`, `set_dataset_stats(s: str)`, `set_theme(name)`. Signals `openRequested`, `refreshRequested`, `themeChanged(name)`, `railCollapseToggled`. | A5, B1, B2, B3, B16 (Open hint) | medium |
...
| **`PlotCanvas`** (single-shared figure widget) | new (or major refactor of `PlotCard` into "card chrome optional" mode) | Hosts N stacked matplotlib subplots in **one** `Figure`, one bottom `MplToolbar`, one `coords` readout. Knows how to switch its renderer between line / bar / scatter / distribution / heatmap (delegates to existing controllers). | ctor `(parent, *, subplots=1)`, `setPlotType(name)`, `addPanel()`, `removePanel(idx)`, `figure`, `canvas`. Signal `plotTypeChanged(name)`. | A3 | **large** |
...
**Items.** **A1, A2, A5, A6 (shell only, contents deferred), B1, B2, B3, B4, B5, B6, B20, B23.**
...
- Titlebar gains: BrandTile + wordmark + `v2.4.1` pill + breadcrumb area + mono file chip + dataset stats + refresh `IconButton` + `Dark/Light` SegmentedControl + Help button + presentation toggle + rail-collapse toggle + primary Open button with `⌘O` `KbdHint`.
...
- `widgets/plot_canvas.py` (from Phase 9) wired to delegate to the existing controllers: `line_graphs_controller`, `bar_plots_controller`, `scatter_controller`, `distribution_controller`, `heatmap_controller`. Switching the subnav swaps the active renderer for *all* subplots simultaneously.
- One persistent `MplToolbar` at the bottom of the canvas. Add `Configure subplots` (delegates to mpl `figure.subplots_adjust` dialog) and `Edit axes/curve` (delegates to mpl `Figure.edit_axes` flow) buttons (B11).
...
- `Add panel` button: stubbed if multi-subplot UX is too large; live if Q3 says yes.
...
- `well_viewer/views/properties_rail_view.py` (new) builds the rail interior:
  ...
  - Seven `CollapsibleSection`s exactly as mockup: Profile & Format, Axes, Legend, Lines & Markers, Grid, Limits & Scale, Layout.
  ...
  - Band controls (SEM/SD/FOV/Spread) re-homed: either a new `Statistics` section between `Profile & Format` and `Axes` (DESIGN_NOTES §6.2) or kept in the existing layout under `Profile & Format` — see Q4.
...
**Items.** **B7, B8 (refinements), B16 (global shortcuts), B17, B18, B19.**
...
| **R3. Theme system needs a second mode.** Re-introducing Light theme touches `theme.qss()` + every QSS rule + matplotlib's `apply_ax_style`. | A5, Phase 10 | If Q2 limits the scope to Dark/Light only (no System), implement Light as a parallel `Colors` namespace and add a `theme.set_mode("dark"|"light")` switch. The matplotlib renderers already understand `screen`/`publication` — Light effectively becomes `screen-light`. Don't try to do System on first pass. |
...
| **R6. ctxbar.subnav semantics are ambiguous.** The mockup has the subnav switching plot type for the whole canvas, but DESIGN_NOTES §2.2 (now being reversed) argued that's the wrong UX. | A3 | Surfaced as Q5; user decides up front. Implementation matches whatever the user picks. |
...
## Section 8 — Open questions for the user (please answer before execution)

These are real ambiguities I need resolved up front; each blocks ≥ 1 phase.

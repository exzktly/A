...
| Concern | Assessment |
|---|---|
| **Cross-selection well exclusivity** (legacy enforces a well in ≤ 1 *rep-set*; groups have no such rule) | The unified model **does not** enforce it (`SELECTIONS_MODEL_CONTRACT.md` §2.3 — a well may be in many selections; that's how bar-plot conditions overlap). The extension therefore does **not** enforce it either — adding a well to selection X leaves it in selection Y. This is a *deliberate* behaviour change from the legacy rep-set UX. If it turns out to confuse users, the Stage-C host wiring can re-add an "exclusive" option for `source=="rep_set"` selections — but the widget stays permissive. **Flag for your sign-off.** |

...
| 6.5.11a — gallery layout proposal (user gate) | **🔒 awaiting approval** (proposed 2026-05-12) | `design/GALLERY_LAYOUT.md` — proposes 8 grouped sections (Form controls / Navigation / Buttons-icons-status / Colour / Overlays / Plate-&-plot / Window chrome / Binding harness), each card stating what it demonstrates (esp. the extensions), host-dependent demos kept as window-opening buttons, plus an in-process `binding_check` result card; keeps `_card()` / `main()` / `build_gallery()` entry points; coverage checklist for 6.5.11b. **No `gallery.py` reorg until you approve.** |
